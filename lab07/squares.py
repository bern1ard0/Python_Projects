...
def drawNestedSquares(size, gap, color, otherColor):
    ...
    # Write me!
    if size<=0:    #base case where the size is completely diminished. 
        pass
    else:        
        #drawing big square
        drawSquare(size, color)
        #repositioning the turtle cursor for recursive step
        fd(gap)
        lt(90)
        fd(gap)
        rt(90)
        #recursive step
        drawNestedSquares(size-2*gap, gap, otherColor, color)
        #bringing back cursor to starting position
        bk(gap)
        lt(90)
        bk(gap)
        rt(90)
